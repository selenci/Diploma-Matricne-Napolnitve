...
\]
\CG{Matriki $A$ in $B$ sta v okviru algoritma ADMM konstantni. Ti na začetku nastavimo na vrednost $A^{(k)}$ in $B^{(k)}$ prejšnjega iterativnega algoritma.} Opazimo lahko, da funkcija upošteva zgolj pogoj $X = W$. Videli bomo, da algoritem pogoj $\proj(X) = \proj(M)$ definira posredno, s popravljanjem znanih vrednosti (korak \eqref{2906-1248} spodaj).%\todo{ali lahko citiram naprej}
...
\end{align*}
Po izreku \ref{1907-2240} \CR{uporabljenem za $X=...$, $Y=...$. $\tau=...$} pa sledi
\[
